function isValidMatrix(matrix) {
  const first = matrix[0].length;
  // console.log(first);
  for (let i = 0; i < matrix.length; i++) {
    // console.log(matrix[i]);
    if (matrix[i].length !== first) {
      return false;
    }
  }
...
function checkCompatiblity(matrixA, matrixB) {
  // console.log(isValidMatrix(matrixA), isValidMatrix(matrixB));
  if (!isValidMatrix(matrixA) || !isValidMatrix(matrixB)) {
    return true;
  }

  // console.log('hi');
  for (const rows in matrixA) {
    const column = getColumn(matrixB, rows);
    console.log(matrixA[rows], column);
    if (matrixA[rows].length !== column.length) {
      return true;
    }
  }
...
function multiplyMatrices(matrixA, matrixB) {
  const resultantArr = [];
  if (checkCompatiblity(matrixA, matrixB)) {
    return NaN;
  }
